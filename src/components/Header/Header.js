...
class Header extends React.Component {
  ...
  componentDidMount() {
    if (sessionStorage.getItem('auth')) {
      ...
      axios.post('www.api3.digimovieplex.com/api/getUserById', { user_id: num_user_id }).then((res) => {
      ...
      if (sessionStorage.getItem('auth')) {
        ...
        if (num_master_id === '2') {
          ...
          return (
            <>
            <ul class="abhiiiiii">
              <li><Link className="green" id="produ_main" onClick={closeNav1} to={"/producer"}>Producer</Link></li>
            </ul>
          ...
          return (
            <>
              <ul class="abhiiiiii">
              <li > <a ><Link  onClick={closeNav1} to={"/"}><span class="green">HOME</span></Link></a></li>
              <li> <a ><Link> <Button className="SELECTION_BUTTON" aria-controls="simple-menu" aria-haspopup="true" onClick={this.handleClick}>
                MOVIES
              </Button></Link>
                <Menu
                  id="simple-menu"
              ...
              <li> <a >  <Link to={"/launch"} onClick={closeNav1} ><Button className="SELECTION_BUTTON">
                Launch
              </Button></Link></a></li>
              </ul>
        ...
        return (
          <>
            ...
            <ul class="abhiiiiii">
              ...
              <li> <a ><input className="search_box" type="text" placeholder="Search.." onChange={(e) => this.setState({ searchItem: e.target.value })} /></a></li>
              <li> <a ><Link className="search_icon" onClick={closeNav1} type="submit" to={(this.state.searchItem) ? `/search/${unescape(encodeURIComponent(this.state.searchItem))}` : ''}><i id="search_bar" class="fa fa-search"></i></Link></a></li>
              <li class="right"> <a > <Link className="subscribe" onClick={() => { this.props.showLogin(false) }}>SIGN UP</Link></a></li>
              <li  class="right"> <a ><Link className="login_hover" onClick={() => { this.props.showLogin(true) }}>LOG IN</Link></a></li>
