...
const SingleContent = ({
  ...
  async function displayRazorpay() {
    if (sessionStorage.getItem('auth')) {
      ...
      // const datavalue = await fetch('www.api3.digimovieplex.com/api/razor_pay', { method: 'POST' }).then((t) =>
      //   t.json()
      // )

      const datavalue = await fetch('www.api3.digimovieplex.com/api/razor_pay', {
        method: 'POST',
        headers: {
      ...
      const options = {
        ...
        handler: function (response) {
          ...
          let redirect = fetch('www.api3.digimovieplex.com/api/insert_transaction', {
            method: 'POST',
            headers: {
              'Accept': 'application/json',
              'Content-Type': 'application/json'
  ...
  //   const { data } = await axios.get(
  //     `www.api3.digimovieplex.com/api/get_Home_allMoviedetails`
  //   );
  ...
  const watchMovieStatus = async () => {
    if (sessionStorage.getItem('auth')) {
      ...
      await axios.post('www.api3.digimovieplex.com/api/get_tran_by_id', data).then((res) => {
    ...
    >
      ...
      <div >
        <img
          ...
          alt={txt_movie_title}
        />
        <h6 className="down_title1">{txt_movie_title}</h6>
        <strong> <h6 className="launchDate">Launch Date {dat_livecast_date}</h6></strong>
        {
          txt_livecast ?
          <button className="ViewNow"> <Link className="view_all" to={`/trailer/${encodeURIComponent(txt_livecast)}`}><strong>View Now</strong></Link>
          </button> : ''
        }
        ...
        <div className="showme">
          ...
          <div className="movie_des">
            ...
            <strong> <p>Ticket Rs. {num_movie_price_inr}</p></strong>
