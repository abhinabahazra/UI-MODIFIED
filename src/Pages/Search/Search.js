...
const Search = () => {
  ...
  const fetchSearch = async () => {
    ...
    axios.post('www.api3.digimovieplex.com/api/getMovieByTitle', { movie_title: searchText }).then((resp) => {
  ...
  const getSearch = () => {
    if (qString) {
      ...
      setSearchText(data);
      axios.post('www.api3.digimovieplex.com/api/getMovieByTitle', { movie_title: data }).then((resp) => {
        //console.log(resp.data);
        if (resp.data.status === "success") {
          setContent(resp.data.response);
        }
