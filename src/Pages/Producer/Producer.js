...
class Producer extends Component {
    ...
    submitHandler = () => {
        if (sessionStorage.getItem('auth')) {
            ...
            console.log(data);
            axios.post('www.api3.digimovieplex.com/api/update_profile', data).then((res) => {
                if (res.data.status === 'success') {
                    ...
                    if (this.state.userDetails.profileImage) {
                        ...
                        axios.post('www.api3.digimovieplex.com/api/file/upload_ProfilePic', formData).then((res1) => {
    ...
    componentDidMount() {
        if (sessionStorage.getItem('auth')) {
            ...
            axios.post('www.api3.digimovieplex.com/api/getUserById', data).then((res) => {
        ...
        if (sessionStorage.getItem('auth')) {
            ...
            axios.post('www.api3.digimovieplex.com/api/getUserById', data).then((res) => {
                ...
                if (res.data.status === "success") {
                    ...
                    if(media && this.state.userDetails.masterId === '2'){
                        swal({
                            className:"mobile_logout",
                            title: "Dear Producer/Filmmaker, Kindly use a desktop/Laptop for accessing this producer/filmmakers section.",
                            // text: "Please switch to Desktop",
                            icon: 'warning',
                            successMode: true,
                            buttons:"LOGOUT"
                          }).then(res => {
