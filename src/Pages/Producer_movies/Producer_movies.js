...
export default function Producer_movies() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('auth')) {
      ...
      fetch("www.api3.digimovieplex.com/api/get_Save_Movie", {
        method: 'POST',
        ...
        <TableBody>
          {movieDetails && movieDetails.map((row, i) => (
            <TableRow key={i}>
              ...
              <TableCell>{row.txt_approval_status && row.txt_approval_status.toUpperCase()}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
