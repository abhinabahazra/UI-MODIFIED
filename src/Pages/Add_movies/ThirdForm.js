...
export class ThirdForm extends Component {
    ...
    onSubmitHandler = async (e) => {
        ...
        await axios.post('www.api3.digimovieplex.com/api/add_movie/', movieDetails).then((res) => {
            if (res.data.status === "success") {
                ...
                this.setState({
                    ...
                    if (parseInt(this.state.movieData.certificate) && this.state.movieData.certificate_link) {
                            ...
                            console.log('Enter');
                            axios.post('www.api3.digimovieplex.com/api/file/upload_CensorCertificate', formData, options).then((resp) => {
    ...
    async componentDidMount() {
        await axios.get('www.api3.digimovieplex.com/api/movie_category_list').then((res) => {
            this.setState({ category: res.data.response })
        });

        await axios.get('www.api3.digimovieplex.com/api/movie_language_list').then((res) => {
            this.setState({ language: res.data.response })
        });

        await axios.get('www.api3.digimovieplex.com/api/genre_user_list').then((res) => {
            this.setState({ genre: res.data.response })
        });

        if(localStorage.getItem('movie_id')){
            let id = localStorage.getItem('movie_id');
            await axios.post('www.api3.digimovieplex.com/api/getMovieDetails',{id}).then((res) => {
