...
class KycForm extends Component {
    ...
    submitHandler = async() => {
        if(sessionStorage.getItem('auth')){
            ...
            await axios.post('www.api3.digimovieplex.com/api/add_kyc_data', kycData).then((res) => {
                console.log(res);
                if(res.data.status==='success'){
                    ...
                    axios.post('www.api3.digimovieplex.com/api/upload_PanImage',panFileData).then((res) => {
                        console.log(res.data)
                    });
                    axios.post('www.api3.digimovieplex.com/api/upload_ChequeImage',chequeFileData).then((res) => {
                        console.log(res.data)
                    });
    ...
    componentDidMount(){
        if(sessionStorage.getItem('auth')){
            ...
            axios.post('www.api3.digimovieplex.com/api/kyc_data',{user_id}).then((res)=>{
                console.log(res.data)
                if(res.data.status === 'success'){
                    this.setState({
                        kycDetails : {
                    ...
                    if(res.data.response.txt_cheque_link!==null && res.data.response.txt_cheque_link!=='' && res.data.response.txt_cheque_link!=='undefined'){
                        this.setState({
                            isFile : {
                                ...this.state.isFile,
                                isChequeFile : `www.api3.digimovieplex.com/${res.data.response.txt_cheque_link}`,
                                isCheque: true
                            }
                        })
                    }
                    ...
                    if(res.data.response.txt_pancard_link!==null && res.data.response.txt_pancard_link!=='' && res.data.response.txt_pancard_link!=='undefined'){
                        this.setState({
                            isFile : {
                                ...this.state.isFile,
                                isPanFile : `www.api3.digimovieplex.com/${res.data.response.txt_pancard_link}`,
                                isPan: true
                            }
                        })
                    }
