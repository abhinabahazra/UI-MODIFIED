...
const Trending = (props) => {
  ...
  const fetchTrending = async () => {
    const { data } = await axios.get(`www.api3.digimovieplex.com/api/get_Home_allMoviedetails`);
    setContent(data.response);
  };

  const fetchRelease = async () => {
    const { data } = await axios.get(`www.api3.digimovieplex.com/api/get_all_active_previous_movie_list`);
    setRelease(data.response);
  };


  const movieFilter = async () => {
    let getMovies = await axios.get(`www.api3.digimovieplex.com/api/get_Home_allMoviedetails`);
    let getCategories = await axios.get(`www.api3.digimovieplex.com/api/movie_category_list`);
