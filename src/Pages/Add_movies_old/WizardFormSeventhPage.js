...
class WizardFormSeventhPage extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitHandler = async (e) => {
        ...
        await axios.post('www.api3.digimovieplex.com/api/add_movie_contract/',sedingData).then((res) => {
            if(res.data.status === 'success'){
                axios.post('www.api3.digimovieplex.com/api/add_approval/',approveData).then((res1) => {
                    localStorage.removeItem('movie_id')
                    if(res1.data.status === 'success'){
                        this.setState({addMovie_msg : res.data.message+'. '+res1.data.message, isDisable: true});
                    }else{
    ...
    getDefaultData = async () =>{
        axios.get('www.api3.digimovieplex.com/api/get_contract_details').then(res => {
            this.setState({
    ...
    render(){
        ...
        return(
            <Form name="page7" {...this.props}>
                ...
                <button id="style_button" type="button" onClick={this.props.prevPage} disabled={this.state.isDisable}>
                    Previous Page
                </button>
...
export default WizardFormSeventhPage;