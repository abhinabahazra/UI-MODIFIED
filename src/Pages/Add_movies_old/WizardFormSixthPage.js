...
class WizardFormSixthPage extends React.Component {
    ...
    onSubmitHandler = async (e) => {
        ...
        await axios.post('www.api3.digimovieplex.com/api/upload_banner1', banner_one, {onUploadProgress: (ev) => {
            ...
            if (response.data.status === "Success") {
                axios.post('www.api3.digimovieplex.com/api/upload_trailer1', trailer_one, {onUploadProgress: (ev) => {
                    if (ev.lengthComputable) {
                        console.log(ev.loaded + " " + ev.total)
                        this.setState({...this.state, upload: (Math.round(ev.loaded * 100 / ev.total))})
                    }
                    ...
                    if (response.data.status === "Success") {
                        axios.post('www.api3.digimovieplex.com/api/upload_movie', movie_upload,{onUploadProgress: (ev) => {
                            if (ev.lengthComputable) {
                            ...
                            if (response.data.status === "Success") {
                                ...
                                axios.post('www.api3.digimovieplex.com/api/upload_trailer2', trailer_two);
